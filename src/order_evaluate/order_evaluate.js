...
class orderEvaluate extends Component {
    constructor(props) {
        ...
        if(currentUrl.indexOf('=')>-1) {
            currentConversationId = currentUrl.substring(currentUrl.indexOf('=') + 1, currentUrl.length)
        }else{
    ...
    componentDidMount() {
        MPConfig()
        document.title = '订单评价'
        let conversationId = this.state.currentConversationId
        let url ='/mp/wepay/'+conversationId;
    ...
    submitEvaluate(orderPrice){
        let param ={
            pathname:'/order/evaluatesuccess',
            query:{satisfy:this.state.satisfy,price:orderPrice,isRefund:this.state.isRefund}
        }
        ...
        let url = '/mp/wepay/evaluation'
        let submitParam = {conversationId:this.state.orderId,isSatisfy:this.state.satisfy === 1?true:false,content:this.state.satisfy === 1?this.state.enjoyValue:this.state.noenjoyValue,evaluateTag:this.state.selectContentText.join('、')}
    ...
    render() {
        const {orderAddress, orderPrice, orderTime, orderType,satisfy,alreayComment,content,selectContent,noComment} = this.state
        var main;
        if(satisfy === 2){
        ...
        return (
            <div className='evaluate'>
                ...
                {
                    alreayComment ? <div className="evaluate-norepeat">
                        <p>已收到您的评价，请不要重复提交哦~</p>

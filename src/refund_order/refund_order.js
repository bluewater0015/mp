...
class RefundOrder extends Component {
    ...
    renderRefundButton(refundStatus,conversationId) {
        if (refundStatus === 0) {
            return (
                <Link to={{pathname: '/feedback_refund',search: '?conversationId='+conversationId}}>
    ...
    toEvaluate(conversationId){
        if(!this.state.disbaled){
            this.props.history.push('/order/evaluation?conversationId='+conversationId)
        }else{
            return false
        }
    }
    evaluateButton(conversationId,isEvaluate){
        if(!isEvaluate){
            return (<span className="evaluate-button" onClick={this.toEvaluate.bind(this,conversationId)}>我要评价</span>)
        }else{
                    ...
                    return <div className="myOrderList" key={index}>
                        ...
                        <div>
                            ...
                            <div className="btnFeedback refundButton">
                                {key.pay !== 0?this.renderRefundButton(key.refundStatus,key.conversationId):''}
                                {this.evaluateButton(key.conversationId,key.isEvaluate)}
                            </div>
                        </div>
                    </div>
                }) : '暂时没有订单.....'}
